...
export default function ProductAddToCart({ product }) {
    ...
    let {DeleteProductFromCart} = useContext(AppContext);
  ...
  return (
    <div className="flex w border border-transparent shadow-lg mr-4 ml-4 p-8 rounded-md flex-col justify-between hover:shadow-2xl hover:border hover:border-blue-50">
      ...
      <div className="mt-4 flex justify-between">
        <p>$ {product.price}</p>
        <svg onClick={()=>{DeleteProductFromCart(product)}} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6 text-red-600">
